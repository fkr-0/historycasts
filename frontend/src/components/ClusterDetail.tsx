...
export default function ClusterDetail(props: {
  ...
  const [yearRange, setYearRange] = useState<[number, number]>(props.initialYearRange ?? yearBounds)
  ...
  const correlations = useMemo(() => {
    ...
    return rows
      .map(r => ({
        ...r,
        otherClusterId: r.cluster_a === props.clusterId ? r.cluster_b : r.cluster_a,
      }))
    ...
    <div className="grid h-full min-h-0 gap-3 overflow-auto">
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <div className="text-xs text-[color:var(--muted)]">
              {cluster.cluster.label || "(unlabeled)"} · {cluster.cluster.n_members} members ·
              podcast {cluster.cluster.podcast_id}
            </div>
          </div>
          <div className="text-xs text-[color:var(--muted)]">
        ...
        <div className="mt-3 grid grid-cols-2 gap-2 text-xs md:grid-cols-5">
          ...
          <div className="rounded-lg border border-[color:var(--border)] bg-[color:var(--surface-2)] p-2">
            ...
            <div className="text-base font-semibold">
              {clusterStats?.temporal_span_years?.toFixed(1) ?? "n/a"}
            </div>
          </div>
          <div className="rounded-lg border border-[color:var(--border)] bg-[color:var(--surface-2)] p-2">
            <div className="text-[color:var(--muted)]">Cohesion</div>
            <div className="text-base font-semibold">
              {clusterStats?.cohesion_proxy?.toFixed(3) ?? "n/a"}
            </div>
          </div>
          <div className="rounded-lg border border-[color:var(--border)] bg-[color:var(--surface-2)] p-2">
            <div className="text-[color:var(--muted)]">Geo Dispersion</div>
            <div className="text-base font-semibold">
              {clusterStats?.geo_dispersion?.toFixed(1) ?? "n/a"}
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="rounded-xl border border-[color:var(--border)] bg-[color:var(--surface)]/60 p-3">
        ...
        <div className="flex flex-wrap gap-2">
          {sortedTerms.slice(0, 30).map(t => {
            ...
            return (
              <button
                ...
                onClick={() =>
                  setActiveTerm(prev =>
                    prev.toLowerCase() === t.phrase.toLowerCase() ? "" : t.phrase
                  )
                }
          ...
          {terms.length === 0 && (
            <div className="text-sm text-[color:var(--muted)]">No terms available.</div>
          )}
        </div>
            ...
            <tbody>
              {entityStats.map(row => (
                <tr
                  ...
                  className="border-t border-[color:var(--border)]/70"
                >
              ...
              >
                <div className="flex items-center justify-between text-sm">
                  ...
                  <span className="text-xs text-[color:var(--muted)]">
                    jaccard {row.jaccard_episode_overlap.toFixed(2)}
                  </span>
                </div>
                <div className="text-xs text-[color:var(--muted)]">
                  cosine {row.cosine_term_similarity.toFixed(2)} · bridge:{" "}
                  {row.bridge_terms.slice(0, 4).join(", ") || "n/a"}
                </div>
              </button>
            ))}
          </div>
