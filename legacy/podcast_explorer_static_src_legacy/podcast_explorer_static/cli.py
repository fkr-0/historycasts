...
def main() -> None:
    ap = argparse.ArgumentParser(prog="podcast-db", description="Build/upgrade podcast DB from RSS feeds")
    ap.add_argument("--db", required=True, help="SQLite database path (created if missing)")
    ap.add_argument("--rss", action="append", required=True, help="Path to RSS/XML file (repeatable)")
